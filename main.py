...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("funcionando")
        await self.highrise.walk_to(Position(16.5 , 1.0 , 26.0 , 
 facing='FrontLeft'))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala")   
        await self.highrise.chat(f"Bienvenue {user.username} dans ❤️•Trouver Votre Amour•❤️.Ecris Help pour les infos utilise de 1 à 95 pour les émotes")
      
        await self.highrise.send_emote("dance-icecream")
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if        message.startswith("Help") or      message.startswith("/help") or      message.startswith("!help") or message.startswith("help"):
            await self.highrise.chat(f"/list  , /drague,  /emotes, /joke  , /emote-id  , /folie , /mariage , /tp @user")
        ...
        if message.startswith("kick"):
          if user.username == "inco_nita":
              ...
              await self.highrise.chat("ce bouffon a été ban")
              return
    ...
    async def on_user_leave(self, user: User) -> None:
        ...
        await self.highrise.chat(f"Au revoir @{user.username} passe une bonne journée❤️")
        await self.highrise.send_emote("emote-charging")